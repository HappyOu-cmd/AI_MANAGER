...
class PromptBuilder:
    ...
    def __init__(self, prompt_file: str = None, tz_template_file: str = None, glossary_file: str = None):
        # Определяем корень проекта
        project_root = Path(__file__).parent.parent.parent
        data_folder = project_root / 'data'
        
        self.prompt_file = Path(prompt_file) if prompt_file else data_folder / "Промпт.txt"
        self.tz_template_file = Path(tz_template_file) if tz_template_file else data_folder / "TZ.json"
        self.glossary_file = Path(glossary_file) if glossary_file else data_folder / "glossary.json"
