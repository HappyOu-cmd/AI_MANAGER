...
# Создаем новую конфигурацию с исключением SSH трафика
...
# Исключаем SSH трафик из VPN (split-tunneling)
# Это позволяет подключаться по SSH напрямую, не через VPN
route-nopull
route 0.0.0.0 0.0.0.0
route 46.183.187.100 255.255.255.255 net_gateway
...
echo ""
echo "📝 Теперь SSH должен работать напрямую, а остальной трафик через VPN"
